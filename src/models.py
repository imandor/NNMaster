...
class SimpleRNNDecoder(object):
    ...
    def fit(self,X_train,y_train):
        ...
        #Add recurrent layer
        model.add(SimpleRNN(self.units,input_shape=(X_train.shape[0],X_train.shape[1]),dropout_W=self.dropout,dropout_U=self.dropout,activation='relu')) #Within recurrent layer, include dropout
...
class LSTMDecoder(object):
    ...
    def fit(self,X_train,y_train):
        ...
        model.add(LSTM(self.units,input_shape=( X_train[0].shape[0],1),dropout_W=self.dropout,dropout_U=self.dropout)) #Within recurrent layer, include dropout
        if self.dropout!=0: model.add(Dropout(self.dropout)) #Dropout some units (recurrent layer output units)
...
class LSTMDecoder_classification(object):
