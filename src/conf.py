...
mlp = Map()
mlp.fc1 = Map()
mlp.fc1.weights = tf.truncated_normal(shape=(56*100,300), stddev=0.01) # 166 * 17
mlp.fc1.activation = tf.nn.relu
mlp.fc2 = Map()
mlp.fc2.weights = tf.truncated_normal(shape=(300, 300), stddev=0.01)
mlp.fc2.activation = tf.nn.relu
mlp.fc3 = Map()
mlp.fc3.weights = tf.truncated_normal(shape=(300, 80 * 30), stddev=0.01)
mlp.fc3.activation = tf.identity
mlp.reshape = Map()
mlp.reshape.shape = [None, 80, 30, 1]
mlp.loss_type = "sigmoid_cross_entropy"
mlp.optimizer = tf.train.AdamOptimizer(0.0001)
